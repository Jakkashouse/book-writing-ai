...
import { PROPOSAL_SYSTEM_PROMPT, createProposalPrompt } from '@/lib/prompts'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimit = checkRateLimit(`proposal-${clientIP}`)
    ...
    const tracker = new TokenTracker(
      ...
      { genre, hasAuthorBio: !!authorBio, hasTOC: !!toc }
    )

    // 프롬프트 생성
    const userPrompt = createProposalPrompt({
      title,
      subtitle,
      genre,
      theme,
    ...
    return NextResponse.json(
      {
        error: error.message || '기획서 생성 중 오류가 발생했습니다.',
      },
      { status: 500 }
    )
  }
