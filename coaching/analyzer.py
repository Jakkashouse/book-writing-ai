...
def classify_submission(scores: dict) -> tuple[str, str, str]:
    """등급 문구는 작가가 읽었을 때 상처받지 않도록 격려 톤으로."""
    total = scores["total"]
    if total >= 75:
        return "S", "기획출판 우선 검토", "대표 직접 검토 → 기획출판 제안 준비"
    if total >= 55:
        return "A", "출간 가능성 매우 높은 원고", "대표 연락 → 맞춤 출간 과정 안내"
    if total >= 35:
        return "B", "출간 잠재력 보유 — 기획 보강 권장", "출간 로드맵 제공 + 재검토 제안"
    return "C", "다듬으면 빛나는 원고", "방향 함께 잡기 → 기획·집필 코칭 추천"
